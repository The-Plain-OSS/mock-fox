...
export class VersionUI {
  ...
  renderVersionItem(version) {
    ...
    return `
      <div class="group relative bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 hover:border-purple-200 dark:hover:border-purple-700 hover:shadow-lg transition-all duration-300 ease-out overflow-hidden">
          ...
          <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <h4 class="font-semibold text-base text-neutral-900 dark:text-neutral-100 mb-2 leading-tight">
                ${version.message || (isManual ? "수동 저장 버전" : "자동 백업")}
              </h4>
            ...
            <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200">
              ...
              <button
                class="version-delete-btn flex items-center justify-center w-10 h-10 rounded-xl bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 hover:text-red-700 transition-all duration-200 hover:scale-105"
                data-version-id="${version.id}"
                title="이 버전 삭제"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  bindVersionEvents() {
    ...
    document.querySelectorAll(".version-delete-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        const versionId = e.target.getAttribute("data-version-id");
        if (versionId) this.deleteVersion(versionId);
      });
    });
  }
  ...
  showToast(message, type = "success") {
    ...
    toast.className = `fixed left-1/2 -translate-x-1/2 bottom-8 px-6 py-4 rounded-2xl ${styles[type]} shadow-2xl backdrop-blur-sm transform transition-all duration-300 ease-out`;
